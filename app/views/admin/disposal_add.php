                ...
                <div class="col-lg-9">
                    ...
                    <div class="admin-card">
                        ...
                        <div class="admin-card-body">
                            <div class="row g-4">
                                <div class="col-md-6">
                                    <label class="form-label fw-medium">Loại phiếu <span
                                            class="text-danger">*</span></label>
                                    <?php $prefillType = !empty($prefill_product) ? 'het_han' : ''; ?>
                                    <select class="form-select" name="loai_phieu" required
                                        ...
                                        <option value="het_han" <?= $prefillType === 'het_han' ? 'selected' : '' ?>>Hết hạn
                                        </option>
                                        <option value="dieu_chinh">Điều chỉnh kiểm kê</option>
                                    </select>
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label fw-medium">Ngày hủy <span
                                            class="text-danger">*</span></label>
                                    <input type="date" class="form-control" name="ngay_huy" value="<?= date('Y-m-d') ?>"
                                ...
                                <div class="col-12">
                                    ...
                                    <textarea class="form-control" name="ly_do" rows="3"
                                        placeholder="Mô tả chi tiết lý do hủy hàng..." required
                                        style="border-radius: 8px; padding: 12px 14px;"><?= htmlspecialchars($prefill_reason ?? '') ?></textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="admin-card-body">
                            ...
                            <div style="position: relative; margin-bottom: 20px;">
                                ...
                                <input type="text" id="productSearch" class="form-control"
                                    placeholder="Tìm sản phẩm theo tên hoặc mã..." style="padding-left: 42px;"
                                    autocomplete="off">
                                <div id="productSuggestions"
                                    style="position: absolute; width: 100%; z-index: 9999; background: #ffffff; border-radius: 8px; margin-top: 8px; max-height: 300px; overflow-y: auto; display: none; box-shadow: 0 4px 20px rgba(0,0,0,0.1); border: 1px solid #e5e7eb; left: 0;">
                                </div>
                            </div>
                            ...
                            <table class="admin-table" id="itemsTable">
                                    ...
                                    <tr id="noItemRow">
                                        <td colspan="6" style="text-align: center; padding: 40px 20px;">
                                            <i class="fas fa-box-open"
                                                style="font-size: 36px; color: var(--admin-text-light); display: block; margin-bottom: 12px;"></i>
                                            <p style="color: var(--admin-text-muted); margin: 0;">Tìm và thêm sản phẩm ở
                                                trên</p>
                                        </td>
                                    </tr>
                                </tbody>
                                <tfoot style="background: #f8fafc;">
                                    <tr>
                                        <td colspan="4" style="text-align: right; font-weight: 600;">TỔNG GIÁ TRỊ HỦY:
                                        </td>
                                        <td id="totalValue"
                                            style="font-weight: 700; color: var(--admin-danger); font-size: 18px;">0đ
                                        </td>
                                        <td></td>
                                    </tr>
                                </tfoot>
                    ...
                    <div class="admin-card" style="position: sticky; top: 20px;">
                        <div class="admin-card-body">
                            ...
                            <div style="display: flex; flex-direction: column; gap: 12px;">
                                ...
                                <a href="<?= BASE_URL ?>/admin/disposals" class="btn-admin-secondary"
                                    style="width: 100%; justify-content: center; padding: 14px;">
                                    <span>Hủy bỏ</span>
                                </a>
                            </div>
        ...
        searchInput.addEventListener('input', function () {
            ...
            searchTimeout = setTimeout(() => {
                ...
                fetch(url)
                    ...
                    .then(data => {
                        if (data.length === 0) {
                            suggestions.innerHTML = '<div style="padding: 20px; text-align: center;"><i class="fas fa-search" style="font-size: 32px; color: #d1d5db; display: block; margin-bottom: 8px;"></i><div style="color: #6b7280; font-size: 14px;">Không tìm thấy sản phẩm</div></div>';
                        } else {
                            suggestions.innerHTML = data.map(p => `
                    <div class="suggestion-item" data-product='${JSON.stringify(p).replace(/'/g, "&apos;")}' style="padding: 12px 16px; cursor: pointer; border-bottom: 1px solid #f1f5f9; display: flex; justify-content: space-between; align-items: center;" onmouseover="this.style.background='#f8fafc'" onmouseout="this.style.background='white'">
                        <div>
