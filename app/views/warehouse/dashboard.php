    ...
    <div class="admin-modern-container">
        ...
        <div class="admin-card mb-4" style="margin-bottom: 24px;">
            <div class="admin-card-body">
                <form id="wh-search-form" class="admin-filter-bar" method="GET" action="<?= BASE_URL ?>/warehouse/dashboard" style="flex-wrap: wrap; gap: 16px;">
                    <div class="form-group">
                        <label>Mã phiếu</label>
                        <input class="form-control" name="ma_phieu" value="<?= htmlspecialchars($filters['ma_phieu']) ?>" placeholder="VD: PNK20251229">
                    </div>
                    <div class="form-group">
                        <label>Người tạo</label>
                        <input class="form-control" name="nguoi_tao" value="<?= htmlspecialchars($filters['nguoi_tao']) ?>" placeholder="Tên nhân viên...">
                    </div>
                    <div class="form-group">
                        <label>Ngày nhập</label>
                        <input class="form-control" type="date" name="ngay_nhap" value="<?= htmlspecialchars($filters['ngay_nhap']) ?>">
                    </div>
        ...
        <div class="admin-card">
            <div class="admin-card-header">
                <h3 class="admin-card-title">Danh sách phiếu nhập <span style="font-size: 14px; font-weight: 400; color: var(--admin-text-muted);">(<?= $total ?>)</span></h3>
            </div>
                    ...
                    <tbody>
                        ...
                        <?php else: ?>
                            <?php foreach ($imports as $row): ?>
                                <tr>
                                    <td>
                                        <span style="color: var(--admin-primary); fontWeight: 600;"><?= htmlspecialchars($row['Ma_hien_thi'] ?? '') ?></span>
                                    </td>
                                    ...
                                    <td style="text-align: center;">
                                        <div style="display: flex; justify-content: center; gap: 8px;">
                                            <button class="btn-icon" type="button" title="Sửa" onclick="openEdit(<?= (int)$row['ID_phieu_nhap'] ?>)">
                                                <i class="fas fa-edit"></i>
                                            </button>
                                            <button class="btn-icon" type="button" title="Xóa" style="color: var(--admin-danger);" onclick="deleteImport(<?= (int)$row['ID_phieu_nhap'] ?>)">
